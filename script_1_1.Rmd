...
Carregamento da tabelas TRANSACTION.dat e TRANSACTION.dat
```{r}
transaction <- read.table("DATA-CRM/TRANSACTION.dat", header = TRUE, sep = ",", dec = ",")
```
Verificação dos dados da tabela transaction, tal como o número de colunas e linhas, bem como se os dados foram corretamente carregados.
...
```
Verificação dos dados da tabela transactionitem, tal como o número de colunas e linhas, bem como se os dados foram corretamente carregados.
...
Carregar tabelas como data.tables e ordenar
```{r}
library(data.table)
...
Fazer 'join' das duas tabelas e calcular o total ('Amount') de cada transação 
```{r}
Result <- merge(transactions, trans_items, all = TRUE)

# Chaves duplicadas, ie, observações correspondentes a transações com mais do que 1 produto
Result[duplicated(Result, by = keycols)]
       

Result[, lapply(.SD, sum), by = keycols, .SDcols='Amount']

Result[, `freq-loc` := .N, by = keycols]


trans_item[, Date := as.IDate(as.character(trans_item$Date), '%Y%m%d')]
DTtransaction[, Date := as.IDate(as.character(DTtransaction$Date), '%Y%m%d')]
```

```{r}

ans <- trans_item[, sum(Amount), by = trans_item$Date]
...
transaction[,  `:=`(
           daystillastord = Sys.Date() - max(as.numeric(transaction$Date)),
           daysbetlastord = if(.N == 1L) "first order" else as.character(max(as.numeric(transaction$Date)) - max(as.numeric(transaction$Date)[as.numeric(transaction$Date) != max(as.numeric(transaction$Date))])),
           meandiff = mean(diff(as.numeric(transaction$Date))),
           OrdsLastFullYear = sum(year(as.numeric(transaction$Date)) == year(Sys.Date()) - 1)
           ),
   by = transaction$CardID][]
```

Frequency
````{r}
library(data.table)
transaction_1 = data.table(transaction)

transaction_1[, `freq-loc` := .N, by = transaction$CardID]
