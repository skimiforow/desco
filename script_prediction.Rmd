...
Numa primeira fase vamos escolher que dados dos clientes vamos analisar e como analisar, bem como, algum tipo de tratamento que seja possível efetuar.
...
Por defeito, o RFM atribui valores de 1 a 5 e apesar destes valores poderem ser configuráveis, consideramos que são suficientes para avaliar os dados dos clientes. As divisões feitas com 5 valores, dividem os valores a cada 20% dos dados.
[Ref. @Birant2011]

Calculo do *Recency*, *Frequency*, *Monetary* a partir da tabela de transacões, resultado do *merge* anterior.
```{r RFM algorithm, echo=TRUE}

# Data definida para a análise, foi escolhida a data de 01-01-2015 pois como os dados são
#de 2014, faria sentido aplicar esta análise nesta altura. 
...
```{r RFM algorithm analysis, echo=TRUE}
...
# A maior mancha de clientes ocorre nos clientes esporádicos e que gastam pouco dinheiro.
...
# Podemos verificar que os cliente que compram mais regularmente gastam menos dinheiro.
...
```{r Avaliacao da Tabela CARD, echo=TRUE}
summary(card)
dim(card)
str(card)
...
```{r Visualizacao grafica dos dados, echo=TRUE}
summary(df_card)
str(df_card)
...
round(prop.table(table(df_card$Responded))*100,digits = 2)

```

Podemos verificar que a proporção de *Sim* e *Não* está muito desbalanceada, pelo que irá ser necessário fazer algo para balancear as proporções, visto que alguns algoritmos não vão conseguir prever o *Sim*, devido à baixa percentagem de ocorrência.

##3. Criação da função para avaliação de modelos de Data-Mining e data-frame para guardar os resultados.
...
A divisão dos dados é feita considerando 70% dos mesmos para treino dos modelos e posteriormente os restantes 30% para a evaliação.
O Oversampling apenas aplicado ao conjunto do treino, serve assim para colocar os valores de *sim* e *não* em percetagens equivalentes na contrução do modelo.
...
```{r C5.0, eval=FALSE, warning=FALSE}
set.seed(123)
...
```{r C5.0 com funcao treino, echo=TRUE, warning=FALSE}
...
```{r C5.0 com funcao treino Winnow, warning=FALSE}
...
##5.Análise do resultado dos modelos de classificação.
...
```{r Avaliacao dos modelos, eval=FALSE, warning=FALSE, include=FALSE}

results.df

barplot(results.df$f1, col = c(1:9), main = "Avaliacao F1", ylim = c(0,0.60), xlim = c(0,10))
...
##7. Para uma segunda classificação dos clientes, forma usados os dados do *score RFM*, em que estes dados númericos forma convertidos para fatores de acordo com uma regra definida abaixo.
...
```{r Holdout stratified para a segunda classificacao}
...
##8. Algoritmos de classificação para segunda classificação
...
Matriz de confusão do método de classificação C5.0

                 | Predicted 
            Real |     Frequent |      Regular | Sporadically |    Row Total | 
    -------------|--------------|--------------|--------------|--------------|
        Frequent |          160 |          416 |          408 |          984 | 
                 |        0.044 |        0.114 |        0.112 |              | 
    -------------|--------------|--------------|--------------|--------------|
         Regular |          220 |          589 |          554 |         1363 | 
                 |        0.060 |        0.161 |        0.151 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Sporadically |          193 |          541 |          577 |         1311 | 
                 |        0.053 |        0.148 |        0.158 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Column Total |          573 |         1546 |         1539 |         3658 | 
    -------------|--------------|--------------|--------------|--------------|
...
Matriz de confusão do método de classificação C5.0 com adpatative boosting
...
Matriz de confusão do método de classificação C5.0 com Naive Bayes.

                 | Predicted 
            Real |     Frequent |      Regular | Sporadically |    Row Total | 
    -------------|--------------|--------------|--------------|--------------|
        Frequent |            0 |          434 |          550 |          984 | 
                 |        0.000 |        0.119 |        0.150 |              | 
    -------------|--------------|--------------|--------------|--------------|
         Regular |            0 |          565 |          798 |         1363 | 
                 |        0.000 |        0.154 |        0.218 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Sporadically |            1 |          524 |          786 |         1311 | 
                 |        0.000 |        0.143 |        0.215 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Column Total |            1 |         1523 |         2134 |         3658 | 
    -------------|--------------|--------------|--------------|--------------|
...
Matriz de confusão do método de classificação K-Nearest neighbours.

                 | Predicted 
            Real |     Frequent |      Regular | Sporadically |    Row Total | 
    -------------|--------------|--------------|--------------|--------------|
        Frequent |           22 |          512 |          450 |          984 | 
                 |        0.006 |        0.140 |        0.123 |              | 
    -------------|--------------|--------------|--------------|--------------|
         Regular |           44 |          683 |          636 |         1363 | 
                 |        0.012 |        0.187 |        0.174 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Sporadically |           41 |          621 |          649 |         1311 | 
                 |        0.011 |        0.170 |        0.177 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Column Total |          107 |         1816 |         1735 |         3658 | 
    -------------|--------------|--------------|--------------|--------------|
...
Matriz de confusão do método de classificação Suport Vector Machine com o method Radial.

                 | Predicted 
            Real |     Frequent |      Regular | Sporadically |    Row Total | 
    -------------|--------------|--------------|--------------|--------------|
        Frequent |           76 |          478 |          430 |          984 | 
                 |        0.021 |        0.131 |        0.118 |              | 
    -------------|--------------|--------------|--------------|--------------|
         Regular |          112 |          651 |          600 |         1363 | 
                 |        0.031 |        0.178 |        0.164 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Sporadically |          104 |          619 |          588 |         1311 | 
                 |        0.028 |        0.169 |        0.161 |              | 
    -------------|--------------|--------------|--------------|--------------|
    Column Total |          292 |         1748 |         1618 |         3658 | 
    -------------|--------------|--------------|--------------|--------------|
...
## Avaliacao dos modelos para previsao do atributo de cliente baseado no RFM.
                                      ...
                                      model   accuracy  recall    precision   f1    kappa   fpr
                              C5.0 Classif.   0.362     0.163     0.279       0.206 0.204   0.161
             C5.0 with adaptative boosting    0.362     0.163     0.279       0.206 0.204   0.161
                               Naive Bayes    0.369     0.000     0.000       0.000 0.253   0.000
                      K-Nearest neighbours    0.370     0.022     0.206       0.040 0.220   0.032
     Support Vector Machine (SVM) - Radial    0.359     0.077     0.260       0.119 0.200   0.082

### Avaliação dos modelos

Devido ao grande desbalanceamento dos dados podemos observar que os valor obtidos de nas medidas de análise são muito baixos, mais baixos do que na primeira análise.

. A medida de *accuracy* encontra-se entre os 36% e 38%
. A medida de *recall* encontra-se entre os 0,0% e 16%
. A medida de *precision* encontra-se entre os 0,0% e 28%
. A medida de *f1* encontra-se entre os 0,0% e 20%
