...
```{r}
test <- df_card[ which(df_card$Responded=='Sim'),]
newData <- df_card
newData <- rbind(newData,test)
table(newData$Responded)
...
train.idx <- createDataPartition(newData$Responded, p = 0.7, list = F)

trainSet <- newData[ train.idx, ]
tstSet <-   newData[-train.idx, ]
...
#TODO: Problema é que apenas de 8,51 % indica o sim.
...
C5model <- train(Responded~., y = trainSet$Responded,
                 method = 'C5.0',
                 metric = 'Accuracy',
                 trControl = ctrl,
                 tuneGrid = grid)
...
Bayesmodel <-train(Responded~., y = trainSet$Responded,
                    method = "nb",
...
resutls.df <- rbind(resutls.df,allcrosstabmeasures(test_Labels,rfpredict,"Random Forest"))
